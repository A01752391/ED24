...
{
    /* Cuando la lista está vacía */
    if (this->empty() || from < 0 || to < from || to >= this->_size) { return; }
    
    /* Declarar los valores iniciales*/
    Node<T> * current = this->_first; // Obtiene el primer elemento
    Node<T> * previous = nullptr; // Se declara como null, ya que no hay elementos previos

    /* Moverse hasta el nodo anterior al rango */
    for (int i = 0; i < from - 1; ++i) {
        previous = current;
        current = current->getNext();
    }

    /* Eliminar el rango de nodos */
    for (int i = from - 1; i <= to - 1; ++i) {
        /* Declarar variable temporal para guardar el nodo a eliminar */
        Node<T> * removenode;

        /* Se guarda el apuntador al nodo a eliminar en la variable REMOVENODE */
        removenode = current;

        /* Mover al siguiente nodo */
        current = current->getNext();

        /* Conectar el nodo anterior al siguiente nodo actual */
        if (previous) {
            previous->setNext(current);
        } else {
            this->_first = current; // Si estamos eliminando desde la cabeza
        }
...
LinkedList<T> * LinkedList<T>::subList(int from, int to)
{
    ...
    /* Cuando la lista está vacía */
    if (this->empty() || from < 0 || to < from || to >= this->_size) {return list; }
    ...
    /* Agregar el rango de nodos a la nueva lista */
    for (int i = from - 1; i <= to - 1; ++i) {
        /* Insertar un elemento en la lista nueva */
        list->insert_back(current ->getInfo() );
        /* Desplazarse al siguiente elemento */
        current = current->getNext();
    }
...
{
    /* Crear una lista vacía para la unión */
    LinkedList<T> * union_list = new LinkedList<T>(); 

    /* Agregar todos los elementos de la lista actual */
    Node<T> * tmp = this->_first; // Obtener una referencia al primer elemento
    while (tmp != nullptr) {
        /* Insertar un elemento en la lista nueva */
        union_list->insert_back(tmp->getInfo() );
        
        /* Desplazarse al siguiente elemento */
        tmp = tmp->getNext();
    }

    /* Agregar todos los elementos de la otra lista */
    tmp = listB->_first; // Obtener una referencia al primer elemento
    while (tmp != nullptr) {
        /* Insertar un elemento en la lista nueva */
        union_list->insert_back(tmp->getInfo() );
...
{
    /* Crear una lista vacía para la diferencia */
    LinkedList<T> * inter_list = new LinkedList<T>(); 

    /* Obtener una referencia al primer elemento de la lista A */
    Node<T> * a_elemento  = this->_first;

    /* Comparar elementos y agregar a la lista */
    while (a_elemento != nullptr) {
        Node<T> * b_elemento  = listB->_first; // Obtener el primer elemento de la lista B
        while (b_elemento != nullptr){
            if (a_elemento->getInfo() == b_elemento->getInfo()){
                inter_list->insert_back(a_elemento->getInfo());
                break;
            }
            b_elemento = b_elemento->getNext();
        }
        a_elemento = a_elemento->getNext();
    }
    return inter_list;
}
...
{
    /* Crear una lista vacía para la diferencia */
    LinkedList<T> * dif_list = new LinkedList<T>(); 
    ...
    while (a_elemento != nullptr) {
        Node<T> * b_elemento  = listB->_first; // Obtener el primer elemento de la lista B
        bool found = false; // Variable para verificar si se encontró el elemento

        while (b_elemento != nullptr){
            /* Verificar si el elemento está en ambas listas */
            if (a_elemento->getInfo() == b_elemento->getInfo()){
                found = true;
                break;
            }
        ...
        /* En caso de no encontrarlo en ambas listas, se añade a la lista de diferencias */
        if (!found) {
            dif_list->insert_back(a_elemento->getInfo());
        }
        a_elemento = a_elemento->getNext();
    }
    return dif_list;
