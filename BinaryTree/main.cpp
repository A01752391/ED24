...
int main(int argc, const char * argv[]) {
    ...
    std::cout << (bst->isBST(root) ? "(V) El árbol es un BST" : "(V) El árbol no es un BST") << std::endl;
    
    /* Determinar la altura de un BST */
    std::cout << "(H) La altura del arbol es: " << bst->height(root) << std::endl;
    ...
    std::cout << "(#) La cantidad de nodos del árbol es: " << bst->count(root) << std::endl;
